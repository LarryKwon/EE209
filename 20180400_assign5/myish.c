...
(getcmdarrayCToken(token)[3]!=NULL)||(getstdinredCToken(token)!=NULL)
...
void execute(char * acLine, char **argv){
   ...
   fds=(int *)calloc(2*(DynArray_getLength(CTokens)-1)+1,sizeof(int));}
         ...
         struct CToken *token=(struct CToken *)DynArray_get(CTokens,i);
                ...
                if(pidarray[i]<0){perror("fork"); exit(-1);}
                    ...
                    if (iFD==-1){perror(argv[0]); _exit(EXIT_FAILURE);}
                        iRet=dup2(iFD,0);
if (iRet==-1){perror(getcmdarrayCToken(token)[0]);_exit(EXIT_FAILURE);}
